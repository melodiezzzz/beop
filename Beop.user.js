...
(function () {
    ...
    (function() {
        ...
        s.textContent =
            ...
            + '#rootdivheader,#rootheader,'
            + '#rootdiv[class=MuiAppBar],'
            + 'div[role=tablist]'
            + '{opacity0!important;pointer-eventsnone!important;}';
    ...
    const DEFAULTS = {
        ...
        chatAccentColor2 ,        gradient end (empty = solid)
         Top Bar
        topBarStyle auto,         autosolidglassdarkgradient
        ...
        topBarShowStats true,       show msgsusers counters
         Messages
        ...
        linkColor ,               link text color override
         UI Chrome
        uiBorderRadius 8,           buttonsinputspanels border-radius px
        scrollbarWidth thin,      thinwidehidden
        scrollbarColor ,          scrollbar thumb color
         Chat Pattern
        chatPattern none,         nonedotsgridlinesdiamonds
        ...
        showNowPlaying true,
         Sound effects
        sfxVolume 0.5,
        sfxJoinUrl httpssafe.cyctorn.techT4lKPHdS.mp3,
        sfxLeaveUrl httpssafe.cyctorn.techVacIrGDE.mp3,
        sfxPairUrl httpssafe.cyctorn.techPVcbyqRi.mp3,
        sfx8ballUrl httpssafe.cyctorn.techgeewKzt2.mp3,
        sfxRollUrl httpssafe.cyctorn.techgeewKzt2.mp3,
         New embeds
        ...
        ttsUserVoices {},    { username_lower { voice, pitch1.0, rate1.0 } }
    ...
    watchForJoinDialog();
     ...
     ─── Utilities ─────────────────────────────────────────────────────────────
    const URL_REGEX = ((httpswww.)[^s']+)g;
    const IMAGE_EXT = .(jpegpnggifwebpavifbmpsvg)(.)$i;
    const VIDEO_EXT = .(webmmp4movogv)(.)$i;
    const AUDIO_EXT = .(mp3oggm4awavflacopus)(.)$i;
    const YOUTUBE_RE = (youtube.comwatch.v=youtu.be)([w-]+)i;
    const TWITTER_RE = (twitter.comx.com)w+status(d+)i;
    const TWITTER_PROFILE_RE = (twitter.comx.com)([A-Za-z0-9_]{1,15})([#].)$i;
    const REDDIT_RE = (reddit.comr[^]+comments[A-Za-z0-9_]+[^s']redd.it[A-Za-z0-9_]+)i;
    const CATBOX_IMG_RE = httpsfiles.catbox.moe[^s']+.(jpegpnggifwebp)i;
    const SEVENTV_RE = httpscdn.7tv.app[^s']+.(jpegpnggifwebpavif)i;
    const IMGUR_RE = https(i.)imgur.com([^s']+)i;
    const TENOR_RE = https(www.)tenor.comview[w-]+i;
    const KLIPY_RE = https(www.)klipy.comgifs[w-]+i;
    const CHAN_RE = httpsboards.4chan(nel).org([a-z0-9]+)thread(d+)(.#p(d+))i;
    const TIKTOK_RE = (https)(www.vm.)tiktok.com(@[^]+video)(d{15,20})i;
    const FOURCHAN_MEDIA_RE = httpsi.4cdn.org[a-z0-9]+d+.w+i;
    const DISCORD_CDN_RE   = https(cdnmedia).discordapp.(comnet)[^s']+.(jpegpnggifwebpavifmp4webm)i;
    ...
    const FXTWITTER_CDN_RE = ^https([w-]+.)fxtwitter.comi;
    const SPOTIFY_RE = httpsopen.spotify.com(trackalbumplaylistartistepisodeshow)([A-Za-z0-9]+)i;
    const GDOC_RE = httpsdocs.google.com(documentspreadsheetspresentation)d([A-Za-z0-9_-]+)i;
    const GITHUB_RE = httpsgithub.com([A-Za-z0-9_.-]+)([A-Za-z0-9_.-]+)((issuespull)(d+))([#][^s])i;
    const STEAM_RE = httpsstore.steampowered.comapp(d+)i;
    const B4K_RE = httpsarch-img.b4k.devi;
...
#ep-panel-closehover { color #fff; background #2a2a2a; }
...
.ep-tabhover { color #ccc; background #252525; }
.ep-tab.active {
...
.ep-rowlast-child { border-bottom none; }
.ep-rowhover { background #202020; }
...
.ep-toggle inputchecked + .ep-toggle-slider { background var(--ep-accent, #e05a2b); }
.ep-toggle inputchecked + .ep-toggle-sliderbefore { transform translateX(16px); background #fff; }
...
.ep-btnhover { opacity 0.85; }
.ep-btnactive { transform scale(0.97); }
.ep-btn.secondary {
    background #2a2a2a; color #bbb;
}
.ep-btn.secondaryhover { background #333; color #fff; }
.ep-btn.danger { background #c0392b; }

 Make MUI accordionpaper panels match the enhancer settings panel background 
...
.ep-native-tabhover,
#ep-tab-trigger-btnhover,
#ep-trigger-btnhover { background rgba(255,255,255,0.14); border-color rgba(255,255,255,0.2); }
...
.MuiDialog-container header.MuiAppBar-root .MuiButton-root.MuiButton-texthas(.MuiButton-icon),
.MuiDialog-container header.MuiAppBar-root .MuiButton-root.MuiButton-texthas(svg) {
    color #fff !important;
    font-size 0 !important;
    line-height 0 !important;
    width 28px !important;
...
.MuiDialog-container header.MuiAppBar-root .MuiButton-root.MuiButton-texthas(.MuiButton-icon) svg,
.MuiDialog-container header.MuiAppBar-root .MuiButton-root.MuiButton-texthas(.MuiButton-icon) .MuiButton-icon {
    color #fff !important;
    fill #fff !important;
...
.MuiDialog-container header.MuiAppBar-root .MuiButton-root.MuiButton-texthas(.MuiButton-icon)before,
.MuiDialog-container header.MuiAppBar-root .MuiButton-root.MuiButton-texthas(svg)before {
    content none !important;
}
.MuiDialog-container header.MuiAppBar-root .MuiButton-root.MuiButton-texthover {
    background rgba(255,255,255,0.16) !important;
    transform translateY(-1px) !important;
}
.MuiDialog-container header.MuiAppBar-root .MuiButton-root.MuiButton-textnot(has(.MuiButton-icon))not(has(svg))before {
    content ✕ !important;
...
.ep-top-btnhover { background rgba(255,255,255,0.14); border-color rgba(255,255,255,0.2); }
#ep-top-bar-img {
...
#ep-hover-header buttonhover { background #3a4a5a; }
#ep-hover-header buttonactive { transform scale(0.93); }
...
.ep-linkhover { color #8fcfff; text-decoration underline; }
.ep-linkvisited { color #b47dff; }
...
lihover .ep-msg-actions { display flex; }
...
.ep-msg-btnhover { background #2a2a2a; color #ccc; }
...
.ep-inline-imghover { opacity 0.85; }
.ep-inline-youtube {
...
@keyframes ep-dot-pulse { 0%,100%{opacity1} 50%{opacity0.25} }
#ep-reply-bar {
...
#ep-reply-bar .ep-reply-cancelhover { color #ccc; }
.ep-reply-quote {
...
.ep-swatchhover { transform scale(1.18); border-color rgba(255,255,255,0.35); }
.ep-swatchactive { transform scale(0.9); }
...
.ep-filter-chiphover { background rgba(255,255,255,0.09); color #bbb; transform translateY(-1px); }
.ep-filter-chip.active {
    background var(--ep-accent-g, var(--ep-accent, #e05a2b)); color #fff;
    border-color var(--ep-accent, #e05a2b);
}
.ep-filter-chipactive { transform scale(0.91); }
...
#ep-filter-user-inputfocus { border-color var(--ep-accent, #e05a2b); background rgba(255,255,255,0.09); width 110px; }
#ep-filter-user-inputplaceholder { color #3d3d3d; }
...
.ep-sectionnot(.active) { opacity 0; pointer-events none; }
.ep-section.active { opacity 1; }
 -- Message action buttons -- 
.ep-msg-btn { transition opacity 0.1s, transform 0.1s, background 0.12s, color 0.12s; }
.ep-msg-btnactive { transform scale(0.84) !important; }
 -- Control bar buttons -- 
.ep-native-tab { transition color 0.15s, background 0.15s, border-color 0.15s, transform 0.1s; }
.ep-native-tabactive { transform scale(0.93); }
 -- Toast enterleave -- 
@keyframes ep-toast-in  { from { opacity0; transformtranslateX(24px) scale(0.92); } to { opacity1; transformtranslateX(0) scale(1); } }
@keyframes ep-toast-out { from { opacity1; transformtranslateX(0) scale(1); } to { opacity0; transformtranslateX(24px) scale(0.92); } }
...
.ep-blocked .ep-blocked-badgehover { background rgba(192,57,43,0.22); }
...
#ep-scroll-pausedhover { opacity 0.8; }
...
.ep-frame-popup {
    ...
    background transparent;
     widthmax-widthmax-height set dynamically in JS based on scene size 
...
#ep-search-inputfocus { outline none; border-color var(--ep-accent, #e05a2b); }
...
@keyframes ep-np-pulse { 0%,100%{opacity1} 50%{opacity0.4} }
...
.ep-gif-btnhover { background rgba(255,255,255,0.08); color #bbb; border-color rgba(255,255,255,0.28); }
...
#ep-gif-searchfocus { outline none; border-color rgba(224,90,43,0.45); background rgba(255,255,255,0.07); }
#ep-gif-searchplaceholder { color #3d3d3d; }
...
.ep-gif-itemhover { opacity 0.78; transform scale(1.025); }
...
.ep-gif-tabhover { color #aaa; background rgba(255,255,255,0.05); }
...
.ep-gif-itemhover .ep-gif-fav-btn { opacity 1; }
.ep-gif-fav-btn.saved { opacity 1 !important; color #f5c518; }
.ep-gif-fav-btnhover { background rgba(0,0,0,0.8); transform scale(1.1); }
.ep-gif-fav-btn.savedhover { color #e05a2b; }
...
#ep-radio-closehover { color #fff; background rgba(0,0,0,0.8); }
...
`;
    ...
    function getChatFontSettings() {
        ...
        const googleFontMap = {
            'inter' 'httpsfonts.googleapis.comcss2family=Interwght@400;600;700&display=swap',
            'roboto' 'httpsfonts.googleapis.comcss2family=Robotowght@400;500;700&display=swap',
            'noto' 'httpsfonts.googleapis.comcss2family=Noto+Sanswght@400;600;700&display=swap',
            'jetbrains' 'httpsfonts.googleapis.comcss2family=JetBrains+Monowght@400;600&display=swap',
        };
        let fontImport = @import url('httpsfonts.googleapis.comcss2family=Interwght@600;700&display=swap');n;
        if (fontChoice === 'custom' && settings.chatFontCustomUrl) {
            fontImport += `@font-face { font-family BeopCustomFont; src url(${settings.chatFontCustomUrl.replace(g, '')}); }n`;
        } else if (googleFontMap[fontChoice]) {
            fontImport += `@import url('${googleFontMap[fontChoice]}');n`;
        }
    ...
    function tagStaticFontZones() {
        ...
        if (!scene) return;
        const classPatterns = textboxtext-boxspeechdialoguetestimonymessage-textbubble-textline-texti;
...
root {
 ...
 ── Sidebar  chat panel ── 
.MuiDrawer-paper,
.MuiGrid2-rootnth-child(2) .MuiStack-root {
...
.MuiGrid2-rootnth-child(2) ul  lihover,
.MuiDrawer-paper ul  lihover {
    background rgba(255,255,255,0.06) !important;
}

 ── Alternating row stripes ── 
#ep-chat-list  linth-child(even),
.MuiGrid2-rootnth-child(2) ul  linth-child(even),
.MuiDrawer-paper ul  linth-child(even) {
    background rgba(255,255,255,0.025) !important;
}

 ── Per-user left-border accent ── 
#ep-chat-list  li[data-ep-user],
.MuiGrid2-rootnth-child(2) ul  li[data-ep-user],
...
.MuiGrid2-rootnth-child(2) ul  li strong,
.MuiDrawer-paper ul  li strong,
.MuiGrid2-rootnth-child(2) ul  li b,
...
.MuiFormControl-rootnot(has(input[name=music i], input[name=sound i])) .MuiInputLabel-root[data-shrink=false],
.MuiAutocomplete-rootnot(has(input[name=music i], input[name=sound i])) .MuiInputLabel-root[data-shrink=false] {
...
.MuiFormControl-rootnot(has(input[name=music i], input[name=sound i])) .MuiInputLabel-root[data-shrink=false].MuiInputLabel-shrink,
.MuiAutocomplete-rootnot(has(input[name=music i], input[name=sound i])) .MuiInputLabel-root[data-shrink=false].MuiInputLabel-shrink {
    transform translate(0, 0) scale(1) !important;
    top 0 !important;
    left 0 !important;
}
...
.MuiGrid2-roothas(#ep-chat-list) ul  li.ep-chat-msg,
...
.MuiInputBase-roothover .MuiOutlinedInput-notchedOutline { border-color #333 !important; }
...
.MuiButton-containedPrimaryhover,
.MuiButton-containedInfohover,
.MuiButton-containedViolethover {
...
.MuiIconButton-roothover,
.MuiToggleButton-roothover {
    background color-mix(in srgb, ${acc} 16%, transparent) !important;
}

 Scrollbars in chat 
.MuiGrid2-rootnth-child(2) -webkit-scrollbar,
.MuiDrawer-paper -webkit-scrollbar { width 4px !important; }
.MuiGrid2-rootnth-child(2) -webkit-scrollbar-thumb,
...
.MuiTypography-caption { color #555 !important; font-size 10px !important; }
 ...
 User list 
.MuiListItem-roothover { background rgba(255,255,255,0.03) !important; }
...
.MuiListItem-root.ep-chat-msghover .MuiListItemText-root,
.MuiListItem-root.ep-chat-msghover .MuiListItemText-root  {
    background transparent !important;
    background-image none !important;
}
.MuiListItem-root.ep-chat-msghover .MuiListItemText-root .MuiTypography-root {
    background transparent !important;
}
...
.ep-composer-toolbarbefore,
.ep-composer-toolbarafter {
    background color-mix(in srgb, ${acc} 18%, rgba(11,11,13,0.99)) !important;
    background-color color-mix(in srgb, ${acc} 18%, rgba(11,11,13,0.99)) !important;
}
...
.MuiGrid2-rootnth-child(2) ul,
.MuiDrawer-paper ul,
.MuiGrid2-roothas(ul) ul {
    background transparent !important;
    border-top none !important;
}
 Dividers 
.MuiGrid2-rootnth-child(2) .MuiDivider-root,
...
.ep-toggle inputchecked + .ep-toggle-slider {
...
.MuiGrid2-rootnth-child(2) a { color ${settings.linkColor} !important; }`  ''}
...
`;
    ...
    const EP_BG_PRESETS = {
       mondstadt 'httpssafe.cyctorn.techmC4ihh8U.jpg',
       prince 'httpssafe.cyctorn.techd6OSlDCF.jpeg',
        hula 'httpssafe.cyctorn.techIsmmuTjJ.jpg',
        cosmo 'httpssafe.cyctorn.techTYPJKG4M.jpeg',
    };
...
div id=ep-panel-header
    h2법h2
    button id=ep-panel-close title=Close (Esc)u00D7button
div
div id=ep-tabs
    button class=ep-tab active data-tab=generalGeneralbutton
    button class=ep-tab data-tab=chatChatbutton
    button class=ep-tab data-tab=mediaMediabutton
    button class=ep-tab data-tab=reskinAppearancebutton
    button class=ep-tab data-tab=advancedAdvancedbutton
    button class=ep-tab data-tab=ttsTTSbutton
div
...
div class=ep-section active data-section=general
    div class=ep-group
        div class=ep-group-titleHighlights & Pingsdiv
    ...
    div
    div class=ep-group
        div class=ep-group-titleSound Effectsdiv
        ${rowRange('sfxVolume', 'SFX Volume', 0, 1, 0.01)}
        ${row('sfxJoinUrl',   'url', 'Join Sound',         'Paste an audio URL u2014 played when someone joins the courtroom')}
    ...
    div
    div class=ep-group
        div class=ep-group-titleHotkeysdiv
        ${row('disableHotkeys', 'toggle', 'Disable ADT Hotkeys', 'Block ADT keys from the game')}
        ${row('enhancerHotkeys', 'toggle', 'Enhancer Hotkeys', 'Ctrl+Shift+E Settings · Ctrl+F Search · Ctrl+K Assets · Ctrl+Shift+S Scroll-lock')}
    div
    div class=ep-group
        div class=ep-group-titleMiscdiv
        ...
        ${row('catboxUpload', 'toggle', 'Drag-to-Upload', 'Drag fileslinks onto inputs to upload to catbox.moe')}
...
div class=ep-section data-section=chat
    div class=ep-group
        div class=ep-group-titleMy Identitydiv
    ...
    div
    div class=ep-group
        div class=ep-group-titleChat Feeddiv
        ${row('autoScroll', 'toggle', 'Auto-scroll', 'Automatically scroll to new messages')}
        ${row('inlineImages', 'toggle', 'Inline Media', 'Render images, YouTube, and TwitterX posts directly in chat')}
    div
    div class=ep-group
        div class=ep-group-titleUser Blockingdiv
        ${row('blockedUsers', 'text', 'Blocked Users', 'Comma-separated usernames to mute')}
    div
    div class=ep-group
        div class=ep-group-titleActionsdiv
        div class=ep-row
            div class=ep-labelspanExport Chat LogspansmallDownload all messages as a filesmalldiv
            div style=displayflex;gap6px;align-itemscenter;
                select id=ep-export-fmt style=background#111;border1px solid #333;color#e8e8e8;border-radius6px;padding4px 6px;font-size12px;
                    option value=txtTXToption
                    option value=jsonJSONoption
                    option value=mdMarkdownoption
                select
                button class=ep-btn secondary id=ep-export-btn style=padding5px 10px;font-size11px;Exportbutton
            div
        div
        div class=ep-row
            div class=ep-labelspanClear Chat LogspansmallWipe the in-memory logsmalldiv
            button class=ep-btn danger id=ep-clear-log-btn style=padding5px 10px;font-size11px;Clearbutton
        div
    div
div
...
div class=ep-section data-section=media
    div class=ep-group
        div class=ep-group-titleUploadsdiv
    ...
    div
    div class=ep-group
        div class=ep-group-titleHover Previewsdiv
        ...
        ${row('fourchanHoverPopup', 'toggle', '4chan Posts', 'Hover over 4chan4channel links to preview posts & images; also embeds inline')}
    div
    div class=ep-group
        div class=ep-group-titleRich Embedsdiv
    ...
    div
    div class=ep-group
        div class=ep-group-titleFrame Popupsdiv
    ...
    div
    div class=ep-group
        div class=ep-group-titleGIF Pickerdiv
...
div class=ep-section data-section=reskin
    div class=ep-group
        div class=ep-group-titleLayout & Displaydiv
        ${row('compactMode', 'toggle', 'Compact Mode', 'Tighter line spacing in chat')}
        ${rowRange('chatFontSize', 'Chat Font Size', 8, 72, 1, 'px')}
        ${row('showTimestamps', 'toggle', 'Timestamps', 'Prepend HHMM to each message')}
    div
    div class=ep-group
        div class=ep-group-titleThemediv
    ...
    div
    div class=ep-group
        div class=ep-group-titleTop Bardiv
    ...
    div
    div class=ep-group
        div class=ep-group-titleMessage Colorsdiv
        ${rowColor('ownMsgBg',   'Own Messages',   '')}
        ${rowColor('mentionBg',  'Mentions',       '')}
        ${rowColor('msgHoverBg', 'Hover',          '')}
        ${rowColor('linkColor',  'Links',          '')}
        small style=color#555;font-size11px;margin-top2px;displayblock;Leave blank to use the accent colour defaults.small
    div
    div class=ep-group
        div class=ep-group-titleUI Chromediv
    ...
    div
    div class=ep-group
        div class=ep-group-titleChat Patterndiv
    ...
    div
    div class=ep-group
        div class=ep-group-titleCustom CSSdiv
        div class=ep-row style=flex-directioncolumn;align-itemsflex-start;gap6px;
            div class=ep-labelspanInject CSSspansmallApplied after all other styles — use !important to override anythingsmalldiv
            textarea id=ep-custom-css-input rows=7 placeholder= your CSS here 
                style=width100%;background#0d0d0f;border1px solid #2a2a2a;color#c8c8c8;border-radius6px;padding8px;font-size11px;font-familymonospace;resizevertical;box-sizingborder-box;line-height1.5;textarea
        div
    div
    div class=ep-group
        div class=ep-group-titleFontdiv
    ...
    div
    div class=ep-group
        div class=ep-group-titleBackgrounddiv
    ...
    div
    div class=ep-group
        div class=ep-group-titleWidgetsdiv
...
div class=ep-section data-section=advanced
    div class=ep-group
        div class=ep-group-titleAbout & Resetdiv
        div class=ep-row
            div class=ep-labelspanVersionspandiv
            span style=color#555;font-size12px;font-familymonospace;v0.8span
        div
        div class=ep-row
            div class=ep-labelspanReset All SettingsspansmallRestores all defaultssmalldiv
            button class=ep-btn danger id=ep-reset-btn style=padding5px 10px;font-size11px;Resetbutton
        div
    div
    div class=ep-group
        div class=ep-group-titleKeyboard Shortcutsdiv
        div style=padding10px 12px;font-size12px;color#777;line-height1.9;
            divkbd style=background#1f1f1f;border1px solid #333;padding1px 6px;border-radius4px;color#cccCtrl+Shift+Ekbd — Open Settingsdiv
            divkbd style=background#1f1f1f;border1px solid #333;padding1px 6px;border-radius4px;color#cccCtrl+Fkbd — Chat Searchdiv
            divkbd style=background#1f1f1f;border1px solid #333;padding1px 6px;border-radius4px;color#cccCtrl+Shift+Skbd — Toggle Auto-scrolldiv
            divkbd style=background#1f1f1f;border1px solid #333;padding1px 6px;border-radius4px;color#cccCtrl+Shift+Bkbd — Toggle Backgrounddiv
            divkbd style=background#1f1f1f;border1px solid #333;padding1px 6px;border-radius4px;color#cccCtrl+Shift+Ckbd — Toggle Compact Modediv
            divkbd style=background#1f1f1f;border1px solid #333;padding1px 6px;border-radius4px;color#cccCtrl+Shift+Xkbd — Export Chat Logdiv
            divkbd style=background#1f1f1f;border1px solid #333;padding1px 6px;border-radius4px;color#cccCtrl+Shift+Nkbd — Toggle Now Playingdiv
            divkbd style=background#1f1f1f;border1px solid #333;padding1px 6px;border-radius4px;color#cccEsckbd — Close panelsdiv
        div
    div
div
...
div class=ep-section data-section=tts
    div class=ep-group
        div class=ep-group-titleText-to-Speechdiv
    ...
    div
    div class=ep-group
        div class=ep-group-titleVoicediv
        div class=ep-row id=ep-tts-voice-row
            div class=ep-labelspanVoicespansmallGoogle voices listed first — enable TTS then reopen panel to populatesmalldiv
            select id=ep-tts-voice-select style=background#111;border1px solid #333;color#e8e8e8;border-radius6px;padding4px 8px;font-size12px;max-width200px;select
        div
    div
    div class=ep-group
        div class=ep-group-titlePer-User Voices &amp; Modulationdiv
        div id=ep-tts-user-list style=displayflex;flex-directioncolumn;gap6px;margin-bottom8px;div
        div style=displaygrid;grid-template-columns1fr auto;gap6px 8px;align-itemscenter;
            div style=grid-column1-1;displayflex;gap6px;align-itemscenter;flex-wrapwrap;
                input id=ep-tts-uv-name type=text placeholder=Username
                    style=background#111;border1px solid #333;color#e8e8e8;border-radius6px;padding4px 8px;font-size12px;flex1;min-width80px;
                select id=ep-tts-uv-voice
                    style=background#111;border1px solid #333;color#e8e8e8;border-radius6px;padding4px 8px;font-size12px;max-width200px;select
            div
            div style=grid-column1-1;displayflex;gap10px;align-itemscenter;flex-wrapwrap;
                label style=font-size11px;color#888;displayflex;align-itemscenter;gap5px;
                    Speed
                    input id=ep-tts-uv-rate type=number value=1.0 min=0.5 max=2 step=0.05
                        style=background#111;border1px solid #333;color#e8e8e8;border-radius6px;padding3px 6px;font-size12px;width60px;
                label
                label style=font-size11px;color#888;displayflex;align-itemscenter;gap5px;
                    Pitch
                    input id=ep-tts-uv-pitch type=number value=1.0 min=0 max=2 step=0.05
                        style=background#111;border1px solid #333;color#e8e8e8;border-radius6px;padding3px 6px;font-size12px;width60px;
                label
                button class=ep-btn secondary id=ep-tts-uv-add style=padding5px 10px;font-size11px;margin-leftauto;Add  Updatebutton
            div
        div
        small style=color#555;font-size11px;margin-top4px;displayblock;Assign a unique voice, speed, and pitch per user — overrides the global defaults for their messages.small
    div
    div class=ep-group
        div class=ep-group-titleTestdiv
        div class=ep-row
            div class=ep-labelspanPreview TTSspansmallSpeak a test phrase nowsmalldiv
            button class=ep-btn secondary id=ep-tts-test-btn style=padding5px 10px;font-size11px;Speak Testbutton
        div
    div
div
...
div id=ep-panel-footer
    span style=font-size11px;color#444;Your settings automatically save.span
    div style=displayflex;gap6px;
        button class=ep-btn secondary id=ep-panel-close-btnClosebutton
    div
div
`;
    ...
    function row(key, type, label, desc) {
        ...
        if (type === 'toggle') {
            return `div class=ep-row data-key=${key}
                div class=ep-labelspan${escapeHTML(label)}span${desc  `small${escapeHTML(desc)}small`  ''}div
                label class=ep-toggleinput type=checkbox name=${key} ${val  'checked'  ''}span class=ep-toggle-sliderspanlabel
            div`;
        }
        return `div class=ep-row data-key=${key}
            div class=ep-labelspan${escapeHTML(label)}span${desc  `small${escapeHTML(desc)}small`  ''}div
    ...
    function rowRange(key, label, min, max, step, desc) {
        ...
        return `div class=ep-row data-key=${key}
            div class=ep-labelspan${escapeHTML(label)}span${desc  `small${escapeHTML(desc)}small`  ''}div
            div style=displayflex;flex-directioncolumn;align-itemsflex-end;gap2px;
                input type=range name=${key} min=${min} max=${max} step=${step} value=${val}
                span class=ep-range-val style=font-size10px;color#666;${val}span
            div
        div`;
    }
    ...
    function rowSwatches(label, presets) {
        const swatchHTML = presets.map(([c1, c2, name]) =
            `button class=ep-swatch title=${name} data-c1=${c1} data-c2=${c2}
             style=backgroundlinear-gradient(135deg,${c1},${c2}); type=buttonbutton`
        ).join('');
    ...
    function rowColor(key, label, defaultVal) {
        ...
        return `div class=ep-row data-key=${key}
            div class=ep-labelspan${escapeHTML(label)}spandiv
    ...
    function _preprocessTTSText(rawText) {
        ...
        text = text
            .replace(blmfaobgi,  'hahahaha')
            .replace(blmaobgi,   'hahaha')
            .replace(broflbgi,   'ha ha ha ha')
            .replace(blolbgi,    'heh')
            .replace(bkekbgi,    'heh')
            .replace(bxdd+bgi,   'heh heh')
            .replace(bxdbgi,     'heh');
        ...
        if (b[A-Z]{2,}b.test(text)) {
            rateHint  = 0.85;
            pitchHint = 1.15;
            text = text.replace(b([A-Z]{2,})bg, w = w.toLowerCase());
        }
    ...
    function _buildTTSItem(username, messageText) {
        ...
        if (!text) return null;
        if (settings.ttsSkipUrls) text = text.replace(httpsS+gi, [link]).trim();
        ...
        text = ppText.replace(.{3,}u2026g,  ellipsis ).replace(s{2,}g,  ).trim();
        if (!text) return null;
        ...
        const perUserRate  = (userCfg && typeof userCfg === object && typeof userCfg.rate  === number)  userCfg.rate   null;
         Apply hint multipliers on top of per-user or global ratepitch
    ...
    function populateTTSVoices() {
        ...
        let voiceOpts = 'option value=Defaultoption';
        if (google.length) {
    ...
    function renderTTSUserVoiceList() {
        ...
        if (!entries.length) {
            container.innerHTML = 'span style=color#555;font-size12px;No per-user voices set.span';
            return;
        }
        container.innerHTML = entries.map(([user, cfg]) = {
            ...
            return `div style=displayflex;align-itemscenter;gap6px;background#1a1a1a;border-radius6px;padding4px 8px;flex-wrapwrap;`
                + `span style=font-size12px;color#e8e8e8;min-width70px;${escapeHTML(user)}span`
                + `span style=font-size11px;color#888;flex1;overflowhidden;text-overflowellipsis;white-spacenowrap; title=${escapeHTML(String(voice))}${escapeHTML(String(voice))}span`
                + `span style=font-size11px;color#666;white-spacenowrap;×${rate.toFixed(2)} ♪${pitch.toFixed(2)}span`
                + `button class=ep-msg-btn ep-tts-uv-remove data-user=${escapeHTML(user)} style=font-size13px;line-height1;padding2px 6px;border-radius4px;&#x2715;button`
                + `div`;
        }).join();
    ...
    function getNativeUserCountFast() {
        ...
        for (const el of labels) {
            ...
            if (!userpeopleparticipantmemberonlineviewer.test(label)) continue;
            const text = (el.textContent  '').trim();
            const m = text.match(b(d{1,3})b);
            if (m) return m[1];
        }
        const top = (document.body.innerText  '').slice(0, 800);
        return top.match((^n)s(d{1,3})s+0s(n$)).[1]  '';
    }
     Auto-scroll
    ...
    function showMentionToast(from, text) {
        ...
        mentionToastEl.innerHTML = `strong style=color#5ab4ff;@mentionstrong from strong${escapeHTML(from)}strongbrspan style=color#999;font-size12px;${escapeHTML(text.slice(0, 80))}span`;
    ...
    function showHighlightToast(from, text) {
        if (!mentionToastEl) return;
        mentionToastEl.innerHTML = `strong style=colorvar(--ep-accent,#e05a2b);highlightstrong${from  ` from strong${escapeHTML(from)}strong`  ''}brspan style=color#999;font-size12px;${escapeHTML(text.slice(0, 100))}span`;
        mentionToastEl.style.display = 'block';
        if (mentionToastTimer) clearTimeout(mentionToastTimer);
        mentionToastTimer = setTimeout(() = { mentionToastEl.style.display = 'none'; }, 5000);
    }
    ...
    function createSearchOverlay() {
        ...
        searchOverlayEl.innerHTML = `
            ...
            span id=ep-search-countspan
            button class=ep-btn secondary id=ep-search-prev style=padding4px 8px;font-size11px;▲button
            button class=ep-btn secondary id=ep-search-next style=padding4px 8px;font-size11px;▼button
            button class=ep-btn secondary id=ep-search-close style=padding4px 8px;font-size11px;✕button
        `;
    ...
    function updateEvidenceFix() {
     ...
     ─── Notification Sound ─────────────────────────────────────────────────────
    const DEFAULT_NOTIFICATION_SOUND = 'httpscdn.freesound.orgpreviews256256113_4772965-lq.mp3';
    ...
    function getOrCreateHover() {
        if (!hoverEl) {
            ...
            hoverEl.innerHTML = `div id=ep-hover-header
                button id=ep-hover-close title=Close with EscESCbutton
            div`;
    ...
    function buildHoverEmbed(url) {
        ...
        if (ytMatch && settings.youtubeHoverPopup) {
            ...
            iframe.src = `httpswww.youtube.comembed${ytMatch[1]}autoplay=1&mute=0`;
            iframe.width = '640'; iframe.height = '360';
            iframe.allow = 'autoplay; encrypted-media; picture-in-picture';
            iframe.style.cssText = 'bordernone;border-radius10px;max-width72vw;max-height78vh;';
            return iframe;
        }
        if (twMatch && settings.twitterHoverPopup) {
            ...
            gmRequest({
                method 'GET',
                url `httpsapi.fxtwitter.comstatus${twMatch[1]}`,
                responseType 'json',
                onload r = {
                    ...
                    text.innerHTML = `div style=padding8px;font-size13px;color#ccc;max-width520px;text-alignleft;
                        strong style=color#5ab4ff;@${escapeHTML(data.author.screen_name  '')}strongbr
                        span${escapeHTML(data.text  '')}span
                        div style=margin-top6px;font-size11px;color#555;${data.created_at  ''}div
                    div`;
                    const wrap = document.createElement('div');
                    wrap.style.cssText = 'displayflex;flex-directioncolumn;gap4px;';
                    const hoverGifVids = [];
        ...
        if (tenorMatch && settings.tenorHoverPopup) {
            ...
            showHoverContent(loading);
            const vidStyle = 'max-width72vw;max-height78vh;border-radius8px;box-shadow0 8px 32px rgba(0,0,0,0.6);';
            ...
            function showTenorHover(mp4Url, fallbackImg) {
             ...
             Strategy 1 page fetch for ogvideo
            gmRequest({
                method 'GET', url,
                headers {
                ...
                onload r = {
                    ...
                    const ogVideoM =
                        html.match(meta[^]+property=[']ogvideo((secure_)url)['][^]+content=[']([^']+)[']i) 
                        html.match(meta[^]+content=[']([^']+)['][^]+property=[']ogvideo((secure_)url)[']i) 
                        html.match([']contentUrl[']ss[']([^']+.mp4[^'])i);
                    const ogImageM =
                        html.match(meta[^]+property=[']ogimage['][^]+content=[']([^']+)[']i) 
                        html.match(meta[^]+content=[']([^']+)['][^]+property=[']ogimage[']i);
                    ...
                    if (mp4Url) {
                        ...
                        gmRequest({
                            method 'GET',
                            url `httpstenor.comoembedurl=${encodeURIComponent(url)}&format=json`,
                            responseType 'json',
                            onload r2 = {
                                const thumb = r2.response.thumbnail_url;
                                const hashM = thumb.match(media.tenor.com([^#]+));
                                const slugM = url.match(tenor.comview([w-]+));
                                const derivedMp4 = (hashM && slugM)  `httpsmedia.tenor.com${hashM[1]}${(thumb.match(([^.]+).[w]+([#]$))  [,''])[1]  slugM.[1]  ''}.mp4`  null;
        ...
        if (klipyHoverMatch && settings.klipyHoverPopup) {
            ...
            const slugM = url.match(klipy.comgifs([w-]+)i);
            if (gmRequest && klipyApiKey && slugM) {
                const q = slugM[1].replace(-d+$, '').replace(-g, ' ');
                gmRequest({
                    method 'GET',
                    url `httpsapi.klipy.comapiv1${klipyApiKey}gifssearchq=${encodeURIComponent(q)}&page=1&per_page=24&customer_id=beop`,
            ...
            if (gmRequest) {
                gmRequest({
                    method 'GET', url,
                    headers { 'User-Agent' 'Mozilla5.0', 'Accept' 'texthtml', 'Sec-Fetch-Mode' 'navigate' },
                    onload r = {
                        if (hoverResolved) return;
                        const html = r.responseText  '';
                        const ogVideoM =
                        ...
                        const ogImageM =
                            html.match(meta[^]+property=[']ogimage['][^]+content=[']([^']+)[']i) 
                            html.match(meta[^]+content=[']([^']+)['][^]+property=[']ogimage[']i);
                        const cdnVideoM = html.match(httpsstatic.klipy.com[^s']+.(mp4webmgif)i);
                        const cdnImgM   = html.match(httpsstatic.klipy.com[^s']+.(webpjpgpng)i);
        ...
        if (chanMatch && settings.fourchanHoverPopup) {
            ...
            gmRequest({
                method 'GET',
                url `httpsa.4cdn.org${board}thread${thread}.json`,
                responseType 'json',
                onload r = {
                    ...
                    const wrap = document.createElement('div');
                    wrap.style.cssText = 'displayflex;flex-directioncolumn;gap6px;max-width480px;';
                    if (post.tim && post.ext) {
                        const img = document.createElement('img');
                        img.src = `httpsi.4cdn.org${board}${post.tim}${post.ext}`;
                        img.style.cssText = 'max-width100%;max-height340px;object-fitcontain;border-radius6px;background#050505;';
                    ...
                    textEl.innerHTML = `div style=text-alignleft;padding4px 0
                        strong style=color#789922;${name}strong
                        span style=color#555;font-size10px;margin-left6px;No.${post.no}span
                        ${post.sub  `div style=color#af0a0f;font-weight700;margin3px 0${escapeHTML(post.sub)}div`  ''}
                        div style=margin-top4px;color#ccc;${escapeHTML(comment).slice(0, 400)}${comment.length  400  '…'  ''}div
                    div`;
        ...
        if (ttMatch) {
            const iframe = document.createElement('iframe');
            iframe.src = `httpswww.tiktok.comembedv2${ttMatch[1]}`;
            iframe.allow = 'autoplay; encrypted-media; picture-in-picture';
            iframe.allowFullscreen = true;
            iframe.style.cssText = 'bordernone;border-radius12px;width340px;height740px;max-width90vw;max-height82vh;';
            return iframe;
        }
        if ((FOURCHAN_MEDIA_RE.test(url)  B4K_RE.test(url)  SEVENTV_RE.test(url)  IMAGE_EXT.test(url)) && settings.imageHoverPopup) {
            const img = document.createElement('img');
            img.style.cssText = 'max-width72vw;max-height78vh;border-radius8px;box-shadow0 8px 32px rgba(0,0,0,0.6);';
        ...
        if (VIDEO_EXT.test(url) && settings.videoHoverPopup) {
            ...
            vid.style.cssText = 'max-width72vw;max-height78vh;border-radius8px;box-shadow0 8px 32px rgba(0,0,0,0.6);';
            return vid;
        }
    ...
    function scrollToBottom() {
        ...
        requestAnimationFrame(() = {
            ...
            if (scrollAnchor.isConnected) {
             ...
             scrollIntoView is blocked by an outer overflowhidden wrapper.
    ...
    function getSystemActionType(li) {
        ...
        if (personaddiconi.test(testId)) return 'join';
        if (personremoveiconi.test(testId)) return 'leave';
        if (handshakepairrequesti.test(testId)) return 'pair';
        return null;
    }

    function getCommandSfxType(primaryText, fallbackText) {
        const candidate = ((primaryText  '').trim()  (fallbackText  '').trim());
        const cmdMatch = candidate.match(^([^n]{1,48}s[-]s)!(8ballrolldice)bi);
    ...
    function processNewMessage(li) {
        ...
        if (username && messageText.startsWith(username)) {
            const stripped = messageText.slice(username.length).replace(^ss, '').trim();
        ...
        if (username && blocked.includes(username.toLowerCase())) {
         ...
         Mark the chat list for CSSsearch targeting
        ...
        setTimeout(() = li.classList.remove('ep-msg-entering'), 400);
         ...
         Frame popup direct images, TwitterX media, and Tenor GIF pages.
        void resolveAndShowFramePopup(li, username, rawText);

         Message context injection (posebubble info from WebSocket)
        if (username) {
        ...
        actions.querySelector('.ep-block-btn').addEventListener('click', () = {
            ...
            if (!li.querySelector('.ep-blocked-badge')) {
                ...
                badge.innerHTML = `⛔ strong${escapeHTML(username)}strong blocked — uundou`;
    ...
    function getMessageContext(li, rawText, username) {
        ...
        const poseMatch = text.match(bpose(sid)s[#-]s(d+)bi)
             li.innerHTML.match(pose(IdID_id)['=s-]+(d+)i);
        if (poseMatch && !parts.some(p = p.startsWith('Pose'))) parts.push(`Pose ${poseMatch[1]}`);
        const bubbleMatch = text.match(bbubbles[#-]s([a-z0-9_-]+)bi)
             li.innerHTML.match(bubble(TypeIdID)['=s-]+([a-z0-9_-]+)i);
    ...
    function hasHighlight(text) {
        ...
        return words.some(w = {
            try {
                 Use word-boundary lookaheadlookbehind so mp does not match camping
    ...
    function processExistingAnchorLinks(li) {
        li.querySelectorAll('a[href]not([data-ep-inline-done])').forEach(function(a) {
            ...
            if (!isMedia) return;
            if (.(mp4webm)$i.test(cleanUrl)) {
                var vid = document.createElement('video');
                vid.controls = true; vid.preload = 'metadata';
                vid.style.cssText = 'displayblock;max-width320px;max-height240px;margin-top4px;border-radius6px;border1px solid #222;background#000;';
                ...
                img.addEventListener('click', function() {
                    ...
                    lg.style.cssText = 'max-width72vw;max-height78vh;border-radius8px;box-shadow0 8px 32px rgba(0,0,0,0.6);';
    ...
    function buildRichFragment(text, username, embeddedUrls = null) {
        ...
        while ((m = URL_REGEX.exec(text)) !== null) {
            ...
            if (settings.inlineImages) {
                ...
                if (ytMatch) {
                    const iframe = document.createElement('iframe');
                    iframe.src = `httpswww.youtube.comembed${ytMatch[1]}`;
                    ...
                    iframe.allowFullscreen = true;
                    iframe.loading = 'lazy';
                    iframe.style.cssText = 'bordernone;border-radius12px;width340px;height580px;max-width100%;displayblock;margin-top6px;';
            ...
            if (settings.inlineImages && (FOURCHAN_MEDIA_RE.test(href)  DISCORD_CDN_RE.test(href)  B4K_RE.test(href)  CATBOX_IMG_RE.test(url)  SEVENTV_RE.test(url)  IMGUR_RE.test(url)  IMAGE_EXT.test(url.split('')[0]))) {
                ...
                if (!embeddedUrls  !embeddedUrls.has(urlKey)) {
                    ...
                    img.addEventListener('click', () = {
                        ...
                        content.style.cssText = 'max-width72vw;max-height78vh;border-radius8px;box-shadow0 8px 32px rgba(0,0,0,0.6);';
                        if (FOURCHAN_MEDIA_RE.test(href)  DISCORD_CDN_RE.test(href)  B4K_RE.test(href)) {
             ...
             Excludes URLs already handled by the embed system above (TwitterTenoretc.)
            if (settings.inlineImages && (VIDEO_EXT.test(url.split('')[0])  (DISCORD_CDN_RE.test(href) && .(mp4webm)i.test(href))) &&
                ...
                if (!embeddedUrls  !embeddedUrls.has(urlKey)) {
                    ...
                    const vid = document.createElement('video');
                    vid.controls = true; vid.preload = 'metadata';
                    vid.style.cssText = 'displayblock;max-width320px;max-height240px;margin-top4px;border-radius6px;border1px solid #222;background#000;cursorpointer;';
    ...
    function buildInlineTweet(statusId, href, frameUser) {
        ...
        wrap.innerHTML = 'div class=ep-tweet-cardLoading tweet...div';
        if (!gmRequest) {
            wrap.innerHTML = 'div class=ep-tweet-cardTweet preview unavailable in this userscript manager.div';
            return wrap;
        }
        gmRequest({
            method 'GET',
            url `httpsapi.fxtwitter.comstatus${statusId}`,
            responseType 'json',
            onload r = {
                const data = r.response.tweet;
                if (!data) {
                    wrap.innerHTML = `div class=ep-tweet-carda class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen tweetadiv`;
                    return;
                }
                ...
                if (media.length  0) {
                    ...
                    media.forEach(m = {
                        if (m.type === 'photo' && m.url) {
                            ...
                            maybeFramePopupFromEmbed(frameUser, videoUrl, isGif  .mp4i.test(videoUrl));
                        }
                    });
     ...
     Attempt to set vid.src directly; on networkCSP error, re-fetch via gmRequest
    ...
    function loadImageViaBlob(img, url) {
        ...
        const extMap = { jpg 'imagejpeg', jpeg 'imagejpeg', png 'imagepng', gif 'imagegif', webp 'imagewebp', bmp 'imagebmp' };
        const ext = (url.split('')[0].split('.').pop()  '').toLowerCase();
        const mime = extMap[ext]  'imagejpeg';
        gmRequest({
            method 'GET', url,
    ...
    function loadVideoViaBlobOrDirect(vid, url, isGif) {
        if (!url) return;
        const mime = .webm($)i.test(url)  'videowebm'  'videomp4';
        const isTwimgUrl = video.twimg.comi.test(url);
    ...
    function pickTwitterVideoUrl(media) {
        ...
        if (media.url && ^httpsi.test(media.url)) return media.url;
        ...
        const mp4Variants = variants.filter(v =
            mp4i.test(v.content_type  '')  .mp4($)i.test(v.url  ''));
    ...
    function getTweetMediaItems(data) {
        const media = data.media  {};
         ...
         to avoid duplicating items that fxtwitter also surfaces in media.photosvideosgifs.
    ...
    function collectMediaUrls(value, out = []) {
        ...
        if (typeof value === 'string') {
            if (^httpsi.test(value)) out.push(value);
            return out;
        }
        ...
        if (typeof value === 'object') {
            Object.entries(value).forEach(([key, item]) = {
                if (^(urlsrcsourcemp4video_urlplayback_url)$i.test(key)) collectMediaUrls(item, out);
                else if (variantvideomediagifi.test(key)) collectMediaUrls(item, out);
            });
        }
        return [...new Set(out)];
    }

     Fetch a Tenor GIF page and extract the animated media URL from ogvideo.
    ...
    function buildInlineTenor(href, frameUser) {
        ...
        if (!gmRequest) {
            wrap.innerHTML = `div class=ep-tweet-carda class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen GIF on Tenoradiv`;
            return wrap;
        }
        ...
        function renderTenorMedia(mp4Url, fallbackImgUrl, title) {
            ...
            const mediaStyle = 'max-width100%;max-height220px;object-fitcontain;border-radius5px;background#050505;displayblock;';
            if (mp4Url) {
                const vid = document.createElement('video');
                vid.autoplay = true; vid.loop = true; vid.muted = true;
                ...
                if (title) { const t = document.createElement('di' + 'v'); t.style.cssText = 'margin-top4px;color#888;font-size11px;'; t.textContent = title; card.appendChild(t); }
                ...
                if (title) { const t = document.createElement('div'); t.style.cssText = 'margin-top4px;color#888;font-size11px;'; t.textContent = title; card.appendChild(t); }
                wrap.innerHTML = ''; wrap.appendChild(card);
            } else {
                wrap.innerHTML = `div class=ep-tweet-carda class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen GIF on Tenoradiv`;
            }
         ...
         Strategy 1 fetch the Tenor page HTML for ogvideo (gives the actual mp4 URL).
        gmRequest({
            method 'GET',
            url href,
            headers {
            ...
            onload r = {
                const html = r.responseText  '';
                 Try ogvideo or ogvideourl in either attribute order
                const ogVideoM =
                    html.match(meta[^]+property=[']ogvideo((secure_)url)['][^]+content=[']([^']+)[']i) 
                    html.match(meta[^]+content=[']([^']+)['][^]+property=[']ogvideo((secure_)url)[']i) 
                    html.match([']contentUrl[']ss[']([^']+.mp4[^'])i);
                const ogTitleM =
                    html.match(meta[^]+property=[']ogtitle['][^]+content=[']([^']+)[']i) 
                    html.match(meta[^]+content=[']([^']+)['][^]+property=[']ogtitle[']i);
                const ogImageM =
                    html.match(meta[^]+property=[']ogimage['][^]+content=[']([^']+)[']i) 
                    html.match(meta[^]+content=[']([^']+)['][^]+property=[']ogimage[']i);
                ...
                if (mp4Url) {
                    ...
                    gmRequest({
                        method 'GET',
                        url `httpstenor.comoembedurl=${encodeURIComponent(href)}&format=json`,
                        responseType 'json',
                        onload r2 = {
                            const thumb = r2.response.thumbnail_url;
                            ...
                            const hashM = thumb.match(media.tenor.com([^#]+));
                            const slugM = href.match(tenor.comview([w-]+));
                            const derivedMp4 = (hashM && slugM)
                                 `httpsmedia.tenor.com${hashM[1]}${(thumb.match(([^.]+).[w]+([#]$))  [,''])[1]  slugM.[1]  ''}.mp4`
                                 null;
    ...
    function buildInlineKlipy(href, frameUser) {
        const wrap = document.createElement('div');
        wrap.className = 'ep-inline-tweet';
        wrap.innerHTML = 'div class=ep-tweet-card style=color#888;Loading GIFu2026div';
        if (!gmRequest) {
            wrap.innerHTML = `div class=ep-tweet-carda class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen GIF on Klipyadiv`;
            return wrap;
        }
        ...
        const klipySlug = href.match(klipy.comgifs([w-]+)i).[1];
        const klipyApiKey = getKlipyKey();
        let resolved = false;
        ...
        function tryKlipyHtmlFallback() {
            ...
            gmRequest({
                ...
                url href,
                headers {
                    'Accept' 'texthtml,applicationxhtml+xml',
                    'Accept-Language' 'en-US,en;q=0.9',
                    'Cache-Control' 'no-cache',
                    'Referer' 'httpsklipy.com'
                },
                onload r = {
                    if (resolved) return;
                    const html = r.responseText  '';
                    ...
                    if (html.includes('challenges.cloudflare.com')  html.includes('_cf_chl_opt')) return;
                     ...
                     1. ogvideo tags (most reliable)
                    const ogM =
                        html.match(meta[^]+property=[']ogvideo((secure_)url)['][^]+content=[']([^']+)[']i) 
                        html.match(meta[^]+content=[']([^']+)['][^]+property=[']ogvideo((secure_)url)[']i);

                     2. JSON-LD VideoObject
                    const jldM = html.match(contentUrlss(https[^]+.(mp4webmgif)[^])i);

                     3. static.klipy.com CDN URLs in the RSC payload (any video format)
                    const cdnVideoM = html.match(httpsstatic.klipy.com[^s']+.mp4i)
                         html.match(httpsstatic.klipy.com[^s']+.webmi)
                         html.match(httpsstatic.klipy.com[^s']+.gifi);

                     4. static.klipy.com CDN image URLs (webpjpg fallback)
                    const cdnImgM = html.match(httpsstatic.klipy.com[^s']+.webpi)
                         html.match(httpsstatic.klipy.com[^s']+.(jpgpng)i);
    ...
    function buildInline4chan(board, thread, postId, href) {
        ...
        wrap.style.cssText = 'border-left3px solid #789922;';
        wrap.innerHTML = 'div style=color#888;font-size12px;Loading post…div';
        if (!gmRequest) {
            wrap.innerHTML = `div class=ep-reddit-title4chan ${board}diva class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen linka`;
            return wrap;
        }
        gmRequest({
            method 'GET',
            ...
            onload r = {
                ...
                const comment = (post.com  '').replace(brsgi, 'n').replace([^]+g, '').trim();
                const isVideo4cdn = post.tim && post.ext && .(webmmp4mov)$i.test(post.ext);
                const isGif4cdn = post.tim && post.ext && .gif$i.test(post.ext);
                let imgHtml = '';
                if (post.tim && post.ext) {
                    const fullUrl = `httpsi.4cdn.org${board}${post.tim}${post.ext}`;
                    const thumbUrl = `httpsi.4cdn.org${board}${post.tim}s.jpg`;
                    if (isVideo4cdn) {
                        imgHtml = `video src=${fullUrl} controls preload=metadata loop
                            style=displayblock;max-width100%;max-height200px;object-fitcontain;border-radius5px;margin-bottom6px;background#050505;video`;
                    } else {
                        imgHtml = `a href=${fullUrl} target=_blank rel=noopener noreferrer
                            img src=${thumbUrl} loading=lazy alt=Post image
                                style=floatleft;max-width80px;max-height80px;object-fitcover;border-radius4px;margin0 8px 4px 0;background#050505;cursorpointer;a`;
                    }
                }
                wrap.innerHTML = `
                    div style=font-size11px;color#555;margin-bottom4px;${board} thread #${thread}${postId  ` › No.${postId}`  ''}div
                    ${post.sub  `div style=color#af0a0f;font-weight700;margin-bottom4px;${escapeHTML(post.sub)}div`  ''}
                    ${imgHtml}
                    span style=color#789922;font-weight700;${escapeHTML(name)}span
                    span style=color#555;font-size10px;margin-left4px;No.${post.no}span
                    div style=margin-top4px;color#ccc;font-size12px;clearboth;white-spacepre-wrap;${escapeHTML(comment).slice(0, 500)}${comment.length  500  '…'  ''}div
                    div style=margin-top6px;a class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen on 4chanadiv`;
            },
            onerror () = {
                wrap.innerHTML = `div class=ep-reddit-title4chan ${board}diva class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen linka`;
            }
    ...
    function buildInlineTwitterProfile(username, href) {
        ...
        wrap.innerHTML = `
            div class=ep-tweet-card
                ...
                div class=ep-tweet-textTwitterX profilediv
                a class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen profilea
            div`;
        return wrap;
    }
    ...
    function buildInlineReddit(href) {
        const wrap = document.createElement('div');
        wrap.className = 'ep-reddit-card';
        wrap.innerHTML = 'div class=ep-reddit-titleLoading Reddit...div';
        if (!gmRequest) {
            wrap.innerHTML = `div class=ep-reddit-titleRedditdiva class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen linka`;
            return wrap;
        }
        const api = href.replace(^https(www.)reddit.com, 'httpswww.reddit.com').replace(([#].)$, '.json');
        gmRequest({
            method 'GET',
            url api,
            headers { 'User-Agent' 'beop-enhancer0.826 (greasyfork.orgscripts578034)' },
            responseType 'json',
            onload r = {
                ...
                wrap.innerHTML = `
                    ...
                    div style=color#888;margin-bottom6px;${escapeHTML(author)} · r${escapeHTML(post.subreddit  '')}div
                    ${thumb  `img src=${escapeHTML(thumb)} loading=lazy alt=Reddit preview style=max-width100%;max-height180px;border-radius5px;background#050505;object-fitcontain;`  ''}
                    div style=margin-top6px;a class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen Redditadiv`;
            },
            onerror () = {
                wrap.innerHTML = `div class=ep-reddit-titleRedditdiva class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen linka`;
            }
    ...
    function buildInlineSpotify(type, id, href) {
        const wrap = document.createElement('div');
        wrap.style.cssText = 'margin-top6px;widthmin(360px,100%);';
        const heightMap = { track 152, episode 152, album 352, playlist 352, artist 352, show 352 };
        const h = heightMap[type.toLowerCase()]  152;
        const iframe = document.createElement('iframe');
        iframe.src = `httpsopen.spotify.comembed${type}${id}utm_source=generator&theme=0`;
        iframe.width = '100%';
        iframe.height = String(h);
        iframe.allow = 'autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture';
        iframe.loading = 'lazy';
        iframe.style.cssText = `bordernone;border-radius12px;displayblock;`;
        wrap.appendChild(iframe);
         MP3 download shortcuts for tracks and episodes
        if (type.toLowerCase() === 'track'  type.toLowerCase() === 'episode') {
            const dlRow = document.createElement('div');
            dlRow.style.cssText = 'margin-top5px;displayflex;gap5px;flex-wrapwrap;align-itemscenter;';
            const label = document.createElement('span');
            label.style.cssText = 'font-size10px;color#555;letter-spacing0.04em;text-transformuppercase;font-weight700;margin-right2px;';
            label.textContent = 'DL MP3';
            dlRow.appendChild(label);
            const converters = [
                { name 'SpotiSaver', url 'httpsspotisaver.neturl=' + encodeURIComponent(href) },
                { name 'spotdl.xyz', url 'httpsspotdl.xyzurl=' + encodeURIComponent(href) },
                { name 'tomp3.cc',   url 'httpstomp3.ccspotify-to-mp3url=' + encodeURIComponent(href) },
            ];
            converters.forEach(function(c) {
                ...
                a.style.cssText = 'font-size11px;color#1db954;backgroundrgba(29,185,84,0.1);border1px solid rgba(29,185,84,0.28);padding2px 8px;border-radius5px;text-decorationnone;white-spacenowrap;';
    ...
    function buildInlineGoogleDoc(type, id, href) {
        ...
        const embedUrl = `httpsdocs.google.com${t}d${id}preview`;
        const iframe = document.createElement('iframe');
        iframe.src = embedUrl;
        iframe.width = '100%';
        iframe.height = '280';
        iframe.loading = 'lazy';
        iframe.style.cssText = 'border1px solid rgba(255,255,255,0.08);border-radius8px;background#0a0a0c;displayblock;';
        const fallback = document.createElement('div');
        fallback.style.cssText = 'margin-top4px;font-size11px;color#666;';
    ...
    function buildInlineGitHub(owner, repo, issueNum, href) {
        const wrap = document.createElement('div');
        wrap.className = 'ep-reddit-card';
        wrap.style.cssText = 'border-left3px solid #6e7681;';
        wrap.innerHTML = 'div style=color#888;font-size12px;Loading GitHub…div';
        if (!gmRequest) {
            wrap.innerHTML = `div style=color#58a6ff;font-weight700;${escapeHTML(owner)}${escapeHTML(repo)}diva class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen on GitHuba`;
            return wrap;
        }
        const apiUrl = issueNum
             `httpsapi.github.comrepos${encodeURIComponent(owner)}${encodeURIComponent(repo)}issues${issueNum}`
             `httpsapi.github.comrepos${encodeURIComponent(owner)}${encodeURIComponent(repo)}`;
        gmRequest({
            method 'GET',
            url apiUrl,
            headers { 'Accept' 'applicationvnd.github+json', 'X-GitHub-Api-Version' '2022-11-28' },
            responseType 'json',
            onload r = {
                const d = r.response;
                if (!d) {
                ...
                if (issueNum) {
                    ...
                    wrap.innerHTML = `
                        div style=font-size10px;color#8b949e;margin-bottom4px;${escapeHTML(owner)}${escapeHTML(repo)}div
                        div style=color#c9d1d9;font-weight700;margin-bottom4px;${escapeHTML(d.title  '')}div
                        div style=margin-bottom6px;
                            span style=displayinline-flex;align-itemscenter;gap4px;font-size10px;padding2px 8px;border-radius999px;border1px solid ${stateColor};color${stateColor};
                                ${stateLabel}
                            span
                            span style=font-size10px;color#8b949e;margin-left6px;#${d.number} by ${escapeHTML(d.user.login  '')}span
                        div
                        ${d.body  `div style=font-size12px;color#8b949e;white-spacepre-wrap;${escapeHTML((d.body  '').slice(0, 300))}${(d.body  '').length  300  '…'  ''}div`  ''}
                        div style=margin-top6px;a class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen on GitHubadiv`;
                } else {
                    ...
                    wrap.innerHTML = `
                        div style=color#58a6ff;font-weight700;margin-bottom4px;${escapeHTML(owner)}${escapeHTML(repo)}div
                        ${d.description  `div style=font-size12px;color#8b949e;margin-bottom6px;${escapeHTML(d.description)}div`  ''}
                        div style=font-size11px;color#666;${stars}${forks  '  ' + forks  ''}${lang  '  ' + lang  ''}div
                        div style=margin-top6px;a class=ep-link href=${escapeHTML(href)} target=_blank rel=noopener noreferrerOpen on GitHubadiv`;
                }
            },
            onerror () = {
    ...
    function buildInlineSteam(appId, href) {
        ...
        wrap.style.cssText = 'margin-top6px;widthmin(360px,100%);';
        const iframe = document.createElement('iframe');
        iframe.src = `httpsstore.steampowered.comwidget${appId}`;
        iframe.width = '100%';
        iframe.height = '190';
        iframe.loading = 'lazy';
        iframe.style.cssText = 'bordernone;border-radius8px;displayblock;background#1b2838;';
    ...
    function dismissAllFramePopups(fast) {
     ...
     Returns the courtroom scene container (the leftmain game area).
    ...
    function findDirectFrameImageUrl(li, rawText) {
        for (const a of li.querySelectorAll('a')) {
            ...
            if (FOURCHAN_MEDIA_RE.test(url)  CATBOX_IMG_RE.test(url)  SEVENTV_RE.test(url)  IMAGE_EXT.test(clean) 
                httpsi.imgur.com[^s']+.(jpegpnggifwebp)i.test(url)) {
                return url;
            }
        }
        ...
        while ((m = URL_REGEX.exec(rawText  '')) !== null) {
            ...
            if (FOURCHAN_MEDIA_RE.test(href)  CATBOX_IMG_RE.test(href)  SEVENTV_RE.test(href)  IMAGE_EXT.test(clean) 
                httpsi.imgur.com[^s']+.(jpegpnggifwebp)i.test(href)) {
                return href;
            }
        }
        return null;
    ...
    function fetchTwitterFrameMedia(href) {
        return new Promise(resolve = {
            ...
            gmRequest({
                method 'GET',
                url `httpsapi.fxtwitter.comstatus${tw[1]}`,
                responseType 'json',
                onload r = {
                    const data = r.response.tweet;
    ...
    async function resolveAndShowFramePopup(li, username, rawText) {
        ...
        if (direct) {
            const isVideo = .(mp4webm)($)i.test(direct)  media.tenor.com.mp4i.test(direct);
    ...
    function showFramePopup(username, mediaUrl, opts = {}) {
        ...
        const isVideo = opts.isVideo  (
            .(mp4webm)($)i.test(imgUrl)  media.tenor.com.mp4i.test(imgUrl)
        );
        ...
        if (sceneOk) {
         ...
         The textspeech box begins at sr.top + portraitH.
    ...
    function initFrameTextAnimation() {
        const TEXT_PAT = textboxtext-boxspeechdialoguetestimonymessage-textbubble-textline-textchattextchat-texti;
        let textBoxObs = null;
    ...
    function startChatObserver(chatList) {
        ...
        if (!scrollAnchor) {
         ...
         (the container may still be emptyunsized when startChatObserver runs).
    ...
    function fetchGiphyGifs(query, cb) {
        ...
        if (!key) { cb([]); return; }
        const url = query
             `httpsapi.giphy.comv1gifssearchapi_key=${key}&q=${encodeURIComponent(query)}&limit=20&rating=g`
             `httpsapi.giphy.comv1gifstrendingapi_key=${key}&limit=20&rating=g`;
        gmRequest({
            method 'GET', url,
            responseType 'json',
    ...
    function fetchKlipyGifs(query, cb) {
        ...
        const url = query
             `httpsapi.klipy.comapiv1${key}gifssearchq=${encodeURIComponent(query)}&page=1&per_page=24&customer_id=${customer}`
             `httpsapi.klipy.comapiv1${key}gifstrendingpage=1&per_page=24&customer_id=${customer}`;
        gmRequest({
            method 'GET', url,
            headers { 'Accept' 'applicationjson' },
            responseType 'json',
    ...
    function loadFavouritesView() {
        ...
        if (!favs.length) {
            grid.innerHTML = 'div class=ep-gif-emptyNo favourites yet.brStar a GIF to save it here.div';
            return;
        }
        grid.innerHTML = '';
    ...
    function createGifPicker() {
        ...
        gifPickerEl.innerHTML = `
            ...
            div
            div id=ep-gif-tabs
                button class=ep-gif-tab active data-tab=searchSearchbutton
                button class=ep-gif-tab data-tab=favs★ Favouritesbutton
            div
            div id=ep-gif-griddiv class=ep-gif-emptyLoading…divdiv
        `;
    ...
    function loadGifResults(query) {
        const grid = document.getElementById('ep-gif-grid');
        if (!grid) return;
        grid.innerHTML = 'div class=ep-gif-emptyLoading…div';
    ...
    function initDragUpload(root) {
        ...
        function upload(data) {
            return new Promise((resolve, reject) = {
                ...
                let uploadUrl = 'httpscatbox.moeuserapi.php';
                if (provider === 'filegarden') {
                    if (!settings.filegardenUserId) {
                        reject(new Error('Add your File Garden user ID in Beop settings.'));
                        return;
                    }
                    uploadUrl = `httpsapi.filegarden.comusers${encodeURIComponent(settings.filegardenUserId)}pipe`;
                    fd.delete('reqtype');
                    fd.delete('url');
                ...
                if (provider === 'cyctorn') {
                    uploadUrl = 'httpssafe.cyctorn.techapiupload';
                    fd.delete('reqtype');
                    fd.delete('url');
                    if (!(data instanceof File)) {
        ...
        function attach(el) {
            ...
            el.addEventListener('dragover', e = {
                ...
                if (!types.includes('Files') && !types.includes('textplain')) return;
                e.preventDefault();
            ...
            el.addEventListener('drop', async e = {
                ...
                const payload = e.dataTransfer.getData('textplain')  e.dataTransfer.files[0];
                if (!payload) return;
                setVal(el, 'Uploading…');
                try {
        ...
        if (!nowPlayingEl) {
            ...
            nowPlayingEl.innerHTML = `
                ...
                span id=ep-now-playing-text
                    span id=ep-now-playing-labelNow Playingspan
    ...
    function parseID3Title(bytes) {
        ...
        while (offset + idLen + (ver === 2  3  6)  Math.min(bytes.length, 10 + tagSize)) {
            ...
            if (!frameId.trim().replace(0g, '')) break;  padding
            offset += idLen;
            let frameSize;
            if (ver === 2) {
            ...
            if (frameId === titleId && frameSize  1) {
                ...
                const clean = text.replace(0g, '').trim();
                return clean  null;
            }
            offset += frameSize;
        }
    ...
    function createFilterBar() {
        ...
        bar.innerHTML = [
            'button class=ep-filter-chip data-filter=mentions@Mebutton',
            'button class=ep-filter-chip data-filter=userUserbutton',
            'input id=ep-filter-user-input type=text placeholder=username… autocomplete=off style=displaynone',
            'span id=ep-filter-countspan',
        ].join('');
    ...
    function getOrCreateReplyBar() {
        ...
        bar.id = 'ep-reply-bar';
        bar.innerHTML = `span id=ep-reply-label↩ Replying to strong id=ep-reply-userstrong em id=ep-reply-previewemspanbutton class=ep-reply-cancel title=Cancel reply×button`;
    ...
    function exportChatLog() {
        ...
        if (fmt === 'json') {
            ...
            mime = 'applicationjson'; ext = 'json';
        } else if (fmt === 'md') {
            content = `# Chat Log — objection.lolnn` + chatLog.map(e = {
                const reply = e.replyTo  ` ↩ ${e.replyTo.user} ${e.replyTo.text.slice(0,80)}n`  '';
                const user = e.user  `${e.user} `  '';
                return `${reply}[${e.ts}] ${user}${e.text}`;
            }).join('nn');
            mime = 'textmarkdown'; ext = 'md';
        } else {
            content = chatLog.map(e = {
                const reply = e.replyTo  `  [↩ ${e.replyTo.user} ${e.replyTo.text.slice(0,60)}]n`  '';
                const user = e.user  `${e.user} `  '';
                return `${reply}[${e.ts}] ${user}${e.text}`;
            }).join('n');
            mime = 'textplain'; ext = 'txt';
        }
        const a = document.createElement('a');
    ...
    function createCourtroomBar() {
        ...
        bar.innerHTML = `
            div style=display flex; align-items center; gap 8px;
                span class=ep-bar-title법 (Beop)span
                span class=ep-bar-metaobjection.lolspan
                div id=ep-host-status
                    span class=ep-host-pill checking id=ep-host-catboxspan class=ep-host-dotspanCatboxspan
                    span class=ep-host-pill checking id=ep-host-gardenspan class=ep-host-dotspanGardenspan
                div
            div
            div id=ep-native-tabs
                ...
                button id=ep-trigger-btn type=button법button
            div
    ...
    function createTopCover() {
        ...
        cover.innerHTML = `
            div class=ep-top-left
                button class=ep-top-btn type=button data-top-action=menu title=Open courtroom menuMenubutton
                strong법strongspan id=ep-room-titlespan
            div
            div class=ep-top-right
                button class=ep-top-btn type=button data-top-action=users title=UsersUsersbutton
                span id=ep-clockspan
                span class=ep-top-statmsgs span id=ep-stat-msgs0spanspan
                span class=ep-top-statusers span id=ep-stat-users—spanspan
            div`;
        ...
        panel.innerHTML = `
            ...
            div id=ep-radio-iframe-wrap
                iframe id=ep-radio-iframe
                    src=httpsradio.cyctorn.tech
                    allow=autoplay
                    sandbox=allow-scripts allow-same-origin allow-forms allow-popupsiframe
            div`;
    ...
    function updateTopBarImage() {
        ...
        img.src = 'httpsfile.gardenaXsgVUzen0_D9K_Fbeop.png';
        img.alt = '';
    ...
    function updateRoomTitle() {
        ...
        const title = document.title
            .replace(s[-]sobjection.lol.$i, '')
            .replace(^objection.lols[-]si, '')
            .trim();
    ...
    function clickNativeTopAction(action) {
        ...
        if (action === 'menu') {
            ...
            target = buttons.find(btn = userpeopleparticipantmemberi.test((btn.getAttribute('aria-label')  '') + ' ' + (btn.title  '') + ' ' + (btn.textContent  '')))  buttons[1];
        } else if (action === 'viewers') {
            target = buttons.find(btn = vieweyespectatorwatchi.test((btn.getAttribute('aria-label')  '') + ' ' + (btn.title  '') + ' ' + (btn.textContent  '')))  buttons[2];
        }
        if (!target) {
    ...
    function updateHostStatus() {
        ...
        pingHost('ep-host-catbox',  'httpscatbox.moe');
        pingHost('ep-host-garden',  'httpsapi.filegarden.com');
    }
    ...
    document.addEventListener('keydown', ev = {
        ...
        if (ev.key === 'Escape') {
            ...
            closeSearch();
            clearHover();
             Close any open native MUI drawermodal (assets menu, etc.)
            const backdrop = document.querySelector('.MuiBackdrop-rootnot([aria-hidden=true])');
    ...
    function maybePlayLaughTrack(data) {
        ...
        if (!laughs+tracki.test(data)) return;
        const now = Date.now();
        if (now - laughTrackLastPlayed  1500) return;
        laughTrackLastPlayed = now;
        const laughTracks = [
            'httpsfile.gardenaXsgVUzen0_D9K_Ftrack1.mp3',
            'httpsfile.gardenaXsgVUzen0_D9K_Ftrack2.mp3',
            'httpsfile.gardenaXsgVUzen0_D9K_Ftrack3.mp3',
            'httpsfile.gardenaXsgVUzen0_D9K_Ftrack4.mp3',
            'httpsfile.gardenaXsgVUzen0_D9K_Ftrack5.mp3'
        ];
    ...
    function initChatSplitter(root) {
        ...
        function splitMessage(text, limit) {
         ...
         Find the send button nearest to a given input (rightmostbelow it)
        ...
        function updateCounter(inputEl) {
            if (!counterEl) {
                ...
                counterEl.id = 'ep-chat-char-counter';
                 ...
                 positionfixed so viewport coords map directly without scroll offsets.
                counterEl.style.cssText = [
                    'positionfixed',
                    'font-size10px', 'font-familymonospace', 'pointer-eventsnone',
                    'z-index9999', 'color#888', 'line-height1',
                    'user-selectnone',
                ].join(';');
